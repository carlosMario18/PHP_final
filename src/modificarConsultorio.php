...
<head>
    ...
    <title>Modificar Consultorio</title>
</head>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($response === false) {
        ...
        if ($data === null && json_last_error() !== JSON_ERROR_NONE) {
            ...
            if ($data) {
                $id = $data->id;
                $redirectUrl = 'modificarConsultorioForm.php?id=' . $id;
                // Redirigir a la página de modificación de cita después de que la página se haya cargado completamente
                echo "<script>document.addEventListener('DOMContentLoaded', function() { window.location.replace('$redirectUrl'); });</script>";
                exit;
            } else {
...
<div class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-md mt-6 w-2/4">
    <h1 class="text-2xl font-bold mb-4">Buscar Consultorio</h1>
    <form id="buscarConsultorioForm" class="mb-4" method="post">
        <div class="mb-4">
